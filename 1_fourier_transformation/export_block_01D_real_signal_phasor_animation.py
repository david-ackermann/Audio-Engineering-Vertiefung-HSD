...
GIF_PATH = OUTPUT_DIR / "real_signal_conjugate_pair.gif"
PREVIEW_PATH = OUTPUT_DIR / "real_signal_conjugate_pair_preview.png"
REAL_VIEW_PATH = OUTPUT_DIR / "real_signal_conjugate_pair_real_view.png"
IMAG_VIEW_PATH = OUTPUT_DIR / "real_signal_conjugate_pair_imag_view.png"
...
def build_animation():
    ...
    z_pos_all = 0.5 * np.exp(1j * OMEGA * helix_times)
    z_neg_all = 0.5 * np.exp(-1j * OMEGA * helix_times)
    ...
    pos_line, = ax_complex.plot([], [], color=POS_COLOR, lw=2.7, label=r"$\frac{1}{2}e^{j\omega t}$")
    neg_line, = ax_complex.plot([], [], color=NEG_COLOR, lw=2.7, label=r"$\frac{1}{2}e^{-j\omega t}$")
    sum_line, = ax_complex.plot([], [], color=SUM_COLOR, lw=2.9, label=r"$\cos(\omega t)$")
    ...
    ax_complex.set_title("Real signal as a phasor sum", pad=12, fontsize=TITLE_SIZE)
    ...
    ax_helix.set_title("Conjugate helices and real sum", pad=12, fontsize=TITLE_SIZE)
    ax_helix.view_init(elev=24, azim=-62)

    def draw_state(current_t):
        z_pos = 0.5 * np.exp(1j * OMEGA * current_t)
        z_neg = 0.5 * np.exp(-1j * OMEGA * current_t)
        z_sum = z_pos + z_neg
    ...
    ax_helix.view_init(elev=90, azim=-90)
    ax_helix.set_title("Real sum in the t-Re plane", pad=12, fontsize=TITLE_SIZE)
    ax_helix.set_zlabel("")
    ax_helix.set_zticks([])
    fig.canvas.draw()
    fig.canvas.print_png(str(REAL_VIEW_PATH.resolve()))

    ax_helix.view_init(elev=0, azim=-90)
    ax_helix.set_title("Imaginary parts in the t-Im plane", pad=12, fontsize=TITLE_SIZE)
    ax_helix.set_ylabel("")
    ax_helix.set_yticks([])
    ax_helix.set_zlabel(r"Im$\{\cdot\}$", color="tab:orange", fontsize=LABEL_SIZE, labelpad=10)
    ax_helix.zaxis.set_major_locator(MultipleLocator(0.5))
    fig.canvas.draw()
    fig.canvas.print_png(str(IMAG_VIEW_PATH.resolve()))
...
def main():
    ...
    print(f"Saved real-view PNG to: {REAL_VIEW_PATH}")
    print(f"Saved imag-view PNG to: {IMAG_VIEW_PATH}")
