...
OUTPUT_DIR = paths.TWO_SIDED_SINE_DIR
DPI = 220
TITLE_SIZE = 24
LABEL_SIZE = 20
TICK_SIZE = 17
...
SIGNAL_VALUES = AMPLITUDE * np.sin(2.0 * np.pi * FREQUENCY_HZ * TIME_VALUES)
SPECTRUM_FREQS = np.array([-FREQUENCY_HZ, FREQUENCY_HZ])
SPECTRUM_MAGS = np.array([AMPLITUDE / 2.0, AMPLITUDE / 2.0])
SPECTRUM_PHASES = np.array([90.0, -90.0])
...
def export_time_signal():
    ...
    style_time_axis(ax, r"Time signal $x(t)=\sin(2\pi f_0 t)$")
...
def export_overview():
    ...
    style_time_axis(axes[0], r"Time signal $x(t)=\sin(2\pi f_0 t)$")
