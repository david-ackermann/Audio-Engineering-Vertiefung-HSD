...
GIF_PATH = OUTPUT_DIR / "phasor_intro_e_minus_jwt.gif"
PREVIEW_PATH = OUTPUT_DIR / "phasor_intro_preview.png"
...
def build_animation():
    ...
    ax_helix.plot(helix_times, helix_values.real, helix_values.imag, color="0.85", lw=1.6)
    helix_trace, = ax_helix.plot([], [], [], color=PHASOR_GREEN, lw=2.1)
    helix_point, = ax_helix.plot([], [], [], "o", color=PHASOR_GREEN, ms=6)
    helix_re_stem, = ax_helix.plot([], [], [], color=RE_COLOR, lw=2.0, alpha=0.95)
    helix_im_stem, = ax_helix.plot([], [], [], color=IM_COLOR, lw=2.0, alpha=0.95)
    ...
    def draw_state(current_t):
        ...
        past_mask = output_times <= current_t
        past_times = output_times[past_mask]
        past_values = np.exp(-1j * OMEGA * past_times)
        ...
        helix_re_stem.set_data_3d([current_t, current_t], [0.0, current_value.real], [0.0, 0.0])
        helix_im_stem.set_data_3d([current_t, current_t], [current_value.real, current_value.real], [0.0, current_value.imag])
    ...
    preview_t = (2.0 * np.pi - np.pi / 4.0) / OMEGA
