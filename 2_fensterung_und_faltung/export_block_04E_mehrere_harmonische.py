...
TIME_COMPONENTS = [(2.0, 1.0), (3.0, 0.5)]
COMPONENTS = []
for frequency_hz, time_amplitude in TIME_COMPONENTS:
    line_amplitude = 0.5 * time_amplitude
    COMPONENTS.extend([(-frequency_hz, line_amplitude), (frequency_hz, line_amplitude)])
COMPONENTS.sort(key=lambda item: item[0])
WINDOW_DURATION = 3.5

STATIC_FRAMES = [-6.0]
GIF_SPLIT_FREQUENCY = 3.0
GIF_VALUES_MIN_TO_PLUS3 = np.round(np.arange(-6.0, GIF_SPLIT_FREQUENCY + 1e-9, 0.05), 3).tolist()
GIF_VALUES_PLUS3_TO_MAX = np.round(np.arange(GIF_SPLIT_FREQUENCY, 6.0 + 1e-9, 0.05), 3).tolist()
GIF_NAME_MIN_TO_PLUS3 = "05_shifted_window_animation_f_minus6_bis_plus3.gif"
STATIC_PLUS3_FILENAME = "06_shifted_window_sampling_f_plus3.png"
GIF_NAME_PLUS3_TO_MAX = "07_shifted_window_animation_f_plus3_bis_f_max.gif"
...
def export_gif(filename, gif_values):
    frames = []
    durations = []
...
def export_last_frame():
    ...
    save_figure(fig, "08_observed_spectrum_full.png")
...
def main():
    ...
    export_gif(GIF_NAME_MIN_TO_PLUS3, GIF_VALUES_MIN_TO_PLUS3)
    save_figure(build_shift_figure(GIF_SPLIT_FREQUENCY), STATIC_PLUS3_FILENAME)
    export_gif(GIF_NAME_PLUS3_TO_MAX, GIF_VALUES_PLUS3_TO_MAX)
    export_last_frame()
