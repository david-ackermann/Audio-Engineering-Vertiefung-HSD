...
SHORT_CONFIG = AnalysisConfig(
    title="Short Hann window",
    output_subdir="01_kurzes_fenster",
    window_length=32,
    hop_size=HOP_SIZE,
)
LONG_CONFIG = AnalysisConfig(
    title="Long Hann window",
...
def build_tradeoff_signal(time_values_s: np.ndarray) -> np.ndarray:
...
SIGNAL_VALUES = build_tradeoff_signal(TIME_VALUES)
DENSE_SIGNAL_VALUES = build_tradeoff_signal(DENSE_TIME_VALUES)
...
def build_analysis_state(config: AnalysisConfig) -> AnalysisState:
    ...
    window_values = np.hanning(config.window_length)
    window_sample_positions_s = np.arange(config.window_length) / FS_HZ

    freq_values_hz = np.fft.rfftfreq(config.window_length, d=1.0 / FS_HZ)
    ...
    stft_coefficients = np.vstack(
        [
            one_sided_amplitude_scaled_coefficients(
                np.fft.rfft(block * window_values) / (config.window_length * coherent_gain),
    ...
    return AnalysisState(
        ...
        delta_f_hz=FS_HZ / config.window_length,
    )


SHORT_STATE = build_analysis_state(SHORT_CONFIG)
LONG_STATE = build_analysis_state(LONG_CONFIG)
DB_REFERENCE_MAGNITUDE = max(
    float(np.max(SHORT_STATE.stft_magnitudes)),
    float(np.max(LONG_STATE.stft_magnitudes)),
)
...
def configure_output_dir(state: AnalysisState) -> None:
    global OUTPUT_DIR
...
def draw_frame_grid(ax, state: AnalysisState, alpha: float = 1.0) -> None:
    for frame_start_s in state.frame_times_s:
        ax.axvline(frame_start_s, color=INACTIVE_GREY, lw=1.15, alpha=alpha, zorder=0)


def shifted_window_dense(state: AnalysisState, frame_index: int) -> np.ndarray:
...
def draw_window_band(ax, state: AnalysisState, frame_index: int, color: str, alpha: float, lw: float) -> None:
...
def draw_previous_windows(ax, state: AnalysisState, previous_frame_indices) -> None:
...
def add_rightward_hop_annotation(ax, state: AnalysisState, source_frame_index: int, target_frame_index: int) -> None:
    source_time_s = state.frame_times_s[source_frame_index]
    target_time_s = state.frame_times_s[target_frame_index]
...
def export_signal_with_empty_spectrum_axis(state: AnalysisState) -> int:
...
def export_frame_spectrum_db_step(
    ...
    style_local_spectrum_db_axis(
        ax_spectrum,
        rf"Local spectrum in dB ($\Delta f={state.delta_f_hz:.1f}$ Hz)",
    )
    save_figure(fig, f"{image_number:02d}_frame_{frame_index}_lokales_spektrum_db.png")
    return image_number + 1


def plot_stft_matrix_db(ax, state: AnalysisState, matrix_data_db: np.ndarray, title: str, *, interpolation: str):
    ...
    ax.set_xticks(state.frame_indices)
...
def export_stft_matrix_progress_db(image_number: int, state: AnalysisState, max_frame_index: int) -> int:
...
def export_stft_matrix_all_db(image_number: int, state: AnalysisState) -> int:
    fig, ax = create_matrix_figure()
    image = plot_stft_matrix_db(
        ax,
        state,
...
def export_spectrogram_db(image_number: int, state: AnalysisState) -> int:
    fig, ax = create_matrix_figure()
    image = ax.imshow(
...
def export_storyboard_for_state(state: AnalysisState) -> None:
...
def main() -> None:
    clear_output_dirs()
    export_storyboard_for_state(SHORT_STATE)
    export_storyboard_for_state(LONG_STATE)
