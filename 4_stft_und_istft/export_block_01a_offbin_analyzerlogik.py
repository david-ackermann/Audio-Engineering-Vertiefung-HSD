...
GROUP_ROOT = OUTPUT_ROOT / "01A_rechteckfenster"
...
EXPORT_CASES = [
    {
        "label": "01A_signal_2_hz_probe_2_hz",
        "n": 16,
        "signal_frequency_hz": 2.0,
        "probe_frequency_hz": 2.0,
    },
    {
        "label": "01A_probe_2_hz",
        "n": 16,
        "signal_frequency_hz": 2.5,
        "probe_frequency_hz": 2.0,
    },
    {
        "label": "01A_probe_3_hz",
        "n": 16,
        "signal_frequency_hz": 2.5,
        "probe_frequency_hz": 3.0,
    },
]

ADDITIONAL_EXPORT_CASES = [
    {
        "label": "01A_probe_2_hz_n32",
        "n": 32,
        "signal_frequency_hz": 2.5,
        "probe_frequency_hz": 2.0,
    },
    {
        "label": "01A_probe_2_hz_n64",
        "n": 64,
        "signal_frequency_hz": 2.5,
        "probe_frequency_hz": 2.0,
    },
    {
        "label": "01A_probe_2_hz_n128",
        "n": 128,
        "signal_frequency_hz": 2.5,
        "probe_frequency_hz": 2.0,
    },
...
RUN_EXPORT_CASES = EXPORT_CASES + ADDITIONAL_EXPORT_CASES
...
def configure_runtime(n_value: int) -> None:
    global N, DELTA_F_HZ, SAMPLE_INDICES, DENSE_INDICES
...
def clear_output_root() -> None:
    ...
    for child in OUTPUT_ROOT.iterdir():
        if child.is_dir() and child.name.startswith("01A_"):
...
def draw_rect_window_background(ax, y_limits) -> None:
    x_window = np.array([0.0, 0.0, N - 1.0, N - 1.0])
    y_window = np.array([0.0, 1.0, 1.0, 0.0])
    ax.fill_between(
        x_window,
        0.0,
        y_window,
...
def prepare_case(case: dict):
    configure_runtime(case.get("n", 16))
    ...
    signal_values = raw_signal_values
    signal_dense = raw_signal_dense
...
def export_windowed_signal_plot(context, output_dir: Path) -> None:
    fig, ax = create_time_figure()
    draw_rect_window_background(ax, SEQUENCE_Y_LIMITS)
    draw_signal_block(ax, context["signal_dense"], context["signal_values"])
    style_sequence_axis(
        ax,
        rf"$x[n]\cdot w[n]$, rectangular window, {freq_text(context['signal_frequency_hz'])} Hz",
        SEQUENCE_Y_LIMITS,
    )
    save_figure(fig, output_dir / "02_signal_mit_rechteckfenster.png", target_size=REFERENCE_TIME_EXPORT_SIZE)
...
def export_negative_frequency_amplitude_no_envelope_plot(context, output_dir: Path) -> None:
    fig, ax = create_spectrum_figure()
    frequencies = context["spectrum_frequencies_hz"]
    amplitudes = context["amplitude_magnitudes"]
    repeated_frequencies, repeated_amplitudes = repeated_spectrum_view(frequencies, amplitudes)
    ...
    save_figure(
        fig,
        output_dir / "16_amplitudenspektrum_negative_frequenzen_ohne_huellkurve.png",
        target_size=REFERENCE_MAG_EXPORT_SIZE,
    )


def export_amplitude_level_db_plot(context, output_dir: Path) -> None:
    fig, ax = create_spectrum_figure()
    frequencies = context["spectrum_frequencies_hz"]
    amplitudes_db = amplitude_to_db(context["amplitude_magnitudes"])
    repeated_frequencies, repeated_levels_db = repeated_spectrum_view(frequencies, amplitudes_db)
    ...
    save_figure(fig, output_dir / "17_amplitudenpegel_db.png", target_size=REFERENCE_MAG_EXPORT_SIZE)
...
def export_one_sided_amplitude_spectrum_plot(context, output_dir: Path, probe_data) -> None:
    ...
    save_figure(fig, output_dir / "18_amplitudenspektrum_einseitig.png", target_size=REFERENCE_MAG_EXPORT_SIZE)


def apply_global_limits(contexts: dict[str, dict]) -> None:
    global_sum_limit = max(context["sum_limit"] for context in contexts.values())
    global_complex_limit = max(context["complex_limit"] for context in contexts.values())
    global_spectrum_limit = max(context["spectrum_limit"] for context in contexts.values())
    global_amplitude_limit = max(context["amplitude_limit"] for context in contexts.values())
    global_one_sided_limit = max(context["one_sided_limit"] for context in contexts.values())

    for context in contexts.values():
        context["sum_limit"] = global_sum_limit
        context["complex_limit"] = global_complex_limit
        context["spectrum_limit"] = global_spectrum_limit
        context["amplitude_limit"] = global_amplitude_limit
        context["one_sided_limit"] = global_one_sided_limit


def export_case(case: dict, context: dict) -> None:
    configure_runtime(context["n"])
    ...
    export_negative_frequency_amplitude_no_envelope_plot(context, output_dir)
    export_amplitude_level_db_plot(context, output_dir)
    export_one_sided_amplitude_spectrum_plot(context, output_dir, probe_data)


def main() -> None:
    clear_output_root()
    contexts = {case["label"]: prepare_case(case) for case in RUN_EXPORT_CASES}
    apply_global_limits(contexts)
    for case in RUN_EXPORT_CASES:
        export_case(case, contexts[case["label"]])
