...
OUTPUT_DIR = (
    ...
    / "02A_stft_als_bewegte_block_dft"
)
...
WINDOW_GREEN = "#66b77a"
ACTIVE_RED = "#d7263d"
COMPONENT_DARK_GREY = "0.32"
COMPONENT_LIGHT_GREY = "0.74"
...
def build_tone_components(time_values_s: np.ndarray):
    ...
    return low_component, high_component, mixed_component


def build_fade_envelope(time_values_s: np.ndarray) -> np.ndarray:
    fade_in = smooth_step(time_values_s, 0.10, 0.20)
    fade_out = 1.0 - smooth_step(time_values_s, DURATION_S - 0.10, 0.20)
    return fade_in * fade_out


def build_component_weights(time_values_s: np.ndarray):
    ...
    fade_envelope = build_fade_envelope(time_values_s)

    low_weight = ((1.0 - switch_12) + 0.58 * switch_23) * fade_envelope
    high_weight = (switch_12 * (1.0 - switch_23) + 0.78 * switch_23) * fade_envelope
    return low_weight, high_weight


def build_signal_contributions(time_values_s: np.ndarray):
    low_component, high_component, _ = build_tone_components(time_values_s)
    low_weight, high_weight = build_component_weights(time_values_s)
    low_contribution = low_weight * low_component
    high_contribution = high_weight * high_component
    return low_contribution, high_contribution, low_contribution + high_contribution


def build_signal(time_values_s: np.ndarray) -> np.ndarray:
    _, _, mixed_signal = build_signal_contributions(time_values_s)
    return mixed_signal


SIGNAL_VALUES = build_signal(TIME_VALUES)
DENSE_SIGNAL_VALUES = build_signal(DENSE_TIME_VALUES)
LOW_TONE_VALUES, HIGH_TONE_VALUES, MIXED_TONE_VALUES = build_signal_contributions(TIME_VALUES)
DENSE_LOW_TONE_VALUES, DENSE_HIGH_TONE_VALUES, DENSE_MIXED_TONE_VALUES = build_signal_contributions(DENSE_TIME_VALUES)
DENSE_LOW_WEIGHT, DENSE_HIGH_WEIGHT = build_component_weights(DENSE_TIME_VALUES)
SIGNAL_LIMIT = 1.15 * max(
    1.0,
    np.max(np.abs(DENSE_SIGNAL_VALUES)),
    np.max(np.abs(DENSE_LOW_TONE_VALUES)),
    np.max(np.abs(DENSE_HIGH_TONE_VALUES)),
    np.max(np.abs(DENSE_MIXED_TONE_VALUES)),
)
...
def full_observation_spectrum(values: np.ndarray) -> np.ndarray:
    coefficients = one_sided_amplitude_scaled_coefficients(
        np.fft.rfft(values) / len(values),
        len(values),
    )
    return np.abs(coefficients)[FULL_VISIBLE_FREQ_MASK]
...
FULL_SPECTRUM_COEFFICIENTS = one_sided_amplitude_scaled_coefficients(
    np.fft.rfft(SIGNAL_VALUES) / TOTAL_SAMPLES,
    TOTAL_SAMPLES,
)
FULL_SPECTRUM_MAGNITUDES = np.abs(FULL_SPECTRUM_COEFFICIENTS)[FULL_VISIBLE_FREQ_MASK]
LOW_TONE_SPECTRUM_MAGNITUDES = full_observation_spectrum(LOW_TONE_VALUES)
HIGH_TONE_SPECTRUM_MAGNITUDES = full_observation_spectrum(HIGH_TONE_VALUES)
MIXED_TONE_SPECTRUM_MAGNITUDES = full_observation_spectrum(MIXED_TONE_VALUES)
MATRIX_TIGHT_BBOXES = {}
...
def draw_component(ax, values: np.ndarray, color: str, *, lw: float, zorder: int):
    ax.plot(DENSE_TIME_VALUES, values, color=color, lw=lw, zorder=zorder)


def draw_low_tone(ax):
    draw_component(
        ax,
        DENSE_LOW_TONE_VALUES,
        COMPONENT_DARK_GREY,
        lw=2.4,
        zorder=2,
    )
...
def draw_low_and_high_tones(ax):
    draw_component(
        ax,
        DENSE_LOW_TONE_VALUES,
        COMPONENT_LIGHT_GREY,
        lw=2.0,
        zorder=1,
    )
    draw_component(
        ax,
        DENSE_HIGH_TONE_VALUES,
        COMPONENT_DARK_GREY,
        lw=2.4,
        zorder=2,
    )
...
def draw_mixed_tone_build(ax):
    ...
    ax.plot(DENSE_TIME_VALUES, DENSE_MIXED_TONE_VALUES, color=SIGNAL_BLACK, lw=2.5, zorder=3)


def draw_full_spectrum(ax, magnitudes: np.ndarray, color: str, *, lw: float = 2.0, marker_size: int = 45, zorder: int = 2):
    ax.vlines(FULL_VISIBLE_FREQ_VALUES_HZ, 0.0, magnitudes, color=color, lw=lw, zorder=zorder)
    ax.scatter(
...
def draw_discrete_samples(ax, sample_times_s: np.ndarray, sample_values: np.ndarray):
    ax.vlines(sample_times_s, 0.0, sample_values, color=SIGNAL_BLUE, lw=1.6, alpha=0.85, zorder=4)
    ax.scatter(
        sample_times_s,
        sample_values,
        s=38,
...
def draw_full_observation_window(ax):
    window_band = np.full_like(DENSE_TIME_VALUES, 0.98)
...
def draw_full_signal_as_active_segment(ax):
    ax.plot(DENSE_TIME_VALUES, DENSE_SIGNAL_VALUES, color="0.72", lw=2.2, zorder=1)
    draw_discrete_samples(ax, TIME_VALUES, SIGNAL_VALUES)
...
def export_low_tone_intro(image_number: int):
    fig, axes = create_pair_figure()
    ax_time, ax_spectrum = axes

    draw_low_tone(ax_time)
    style_time_axis(ax_time, rf"Low tone: $f={LOW_FREQUENCY_HZ:.0f}$ Hz")
    style_full_spectrum_axis(ax_spectrum, "Frequency axis")

    save_figure(fig, f"{image_number:02d}_tiefer_ton_und_frequenzachse.png")


def export_low_high_tone_intro(image_number: int):
    fig, axes = create_pair_figure()
    ax_time, ax_spectrum = axes

    draw_low_and_high_tones(ax_time)
    style_time_axis(ax_time, rf"High tone: $f={HIGH_FREQUENCY_HZ:.0f}$ Hz")
    style_full_spectrum_axis(ax_spectrum, "Frequency axis")

    save_figure(fig, f"{image_number:02d}_tiefer_und_hoeherer_ton.png")


def export_mixed_tone_intro(image_number: int):
    fig, axes = create_pair_figure()
    ax_time, ax_spectrum = axes

    draw_mixed_tone_build(ax_time)
    style_time_axis(ax_time, "Mixed signal")
    style_full_spectrum_axis(ax_spectrum, "Frequency axis")

    save_figure(fig, f"{image_number:02d}_mixed_signal_und_frequenzachse.png")


def export_full_window_spectrum(image_number: int):
    fig, axes = create_pair_figure()
    ax_time, ax_spectrum = axes

    draw_full_observation_window(ax_time)
    draw_full_signal_as_active_segment(ax_time)
    style_time_axis(ax_time, r"One rectangular window over $T$")
    ...
    save_figure(fig, f"{image_number:02d}_rechteckfenster_ueber_T_und_spektrum.png")
...
def export_single_analysis_window():
    ...
    style_time_axis(ax, "One rectangular window")
...
def export_overlapping_frames():
    ...
    draw_signal(ax)
    style_time_axis(ax, "Many overlapping rectangular frames")
...
def export_stft_matrix_progress(image_number: int, max_frame_index: int):
    ...
    save_figure(fig, f"{image_number:02d}_stft_matrix_bis_frame_{max_frame_index}.png", tight=True)
...
def main():
    clear_output_dir()
    image_number = 1
    export_low_tone_intro(image_number)
    image_number += 1
    export_low_high_tone_intro(image_number)
    image_number += 1
    export_mixed_tone_intro(image_number)
    image_number += 1
    export_full_window_spectrum(image_number)
    shown_frames = []
    image_number += 1
