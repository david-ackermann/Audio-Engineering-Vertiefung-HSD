...
OUTPUT_DIR = LECTURE_DIR / "png_storyboards" / "02_am_modulation" / "02b_ringmodulation_audible_low_band"
AUDIO_OUTPUT_DIR = LECTURE_DIR / "audio_exports" / "02_am_modulation" / "02b_ringmodulation_audible_low_band"
...
CARRIER_LOWPASS_HZ = 800.0
CARRIER_LOWPASS_TRANSITION_HZ = 200.0
RING_MODULATION_FREQUENCY_HZ = 2_400.0
SPECTRUM_FFT_LENGTH = 2**18
SPECTRUM_DB_FLOOR = -85.0
AUDIO_EXPORT_GAIN = 0.95
SPECTRUM_X_LIMIT_KHZ = 24.0
SPECTRUM_TICK_STEP_KHZ = 6.0
...
def write_wav_mono(path: Path, sample_rate_hz: float, signal: np.ndarray) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    normalized = np.asarray(signal, dtype=np.float64)
    normalized = normalized / max(float(np.max(np.abs(normalized))), np.finfo(float).eps)
    audio_i16 = np.round(np.clip(normalized * AUDIO_EXPORT_GAIN, -1.0, 1.0) * 32767.0).astype("<i2")

    with wave.open(str(path), "wb") as handle:
        handle.setnchannels(1)
        handle.setsampwidth(2)
        handle.setframerate(int(round(sample_rate_hz)))
...
def style_spectrum_axis(ax: plt.Axes) -> None:
    ...
    ax.tick_params(axis="both", labelsize=TICK_SIZE)
    ax.grid(True, color=GRID_GREY, alpha=0.25)
    ax.set_xlim(-SPECTRUM_X_LIMIT_KHZ, SPECTRUM_X_LIMIT_KHZ)
    ax.set_ylim(SPECTRUM_DB_FLOOR, 3.0)
    ax.set_xticks(np.arange(-SPECTRUM_X_LIMIT_KHZ, SPECTRUM_X_LIMIT_KHZ + 0.1, SPECTRUM_TICK_STEP_KHZ))
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def save_carrier_spectrum(raw_carrier: np.ndarray, carrier: np.ndarray, sample_rate_hz: float) -> Path:
    ...
    ax.axvline(CARRIER_LOWPASS_HZ / 1000.0, color=REFERENCE_GREY, lw=1.4, ls="--")
    ax.set_title("Low-passed carrier spectrum", fontsize=TITLE_SIZE, pad=14)
    style_spectrum_axis(ax)
    fig.subplots_adjust(left=0.10, right=0.98, bottom=0.18, top=0.86)
    path = OUTPUT_DIR / "04_low_passed_carrier_spectrum_twosided.png"
    fig.savefig(path, dpi=DPI, facecolor="white")
    plt.close(fig)
    return path
...
def save_modulation_time_zoom() -> Path:
    ...
    ax.set_title("2.4 kHz sine", fontsize=TITLE_SIZE, pad=14)
    ax.set_xlim(0.0, zoom_duration_s * 1000.0)
    ax.set_ylim(-1.05, 1.05)
    ax.set_xticks(np.arange(0.0, zoom_duration_s * 1000.0 + 0.001, 0.5))
    style_time_axis(ax, xlabel="Time in ms", ylabel="Amplitude")
    fig.subplots_adjust(left=0.10, right=0.98, bottom=0.18, top=0.86)
    path = OUTPUT_DIR / "05_modulation_signal_2p4khz_time_zoom.png"
    fig.savefig(path, dpi=DPI, facecolor="white")
    plt.close(fig)
    return path
...
def save_modulation_spectrum() -> Path:
    ...
    style_spectrum_axis(ax)
    fig.subplots_adjust(left=0.10, right=0.98, bottom=0.18, top=0.86)
    path = OUTPUT_DIR / "06_modulation_signal_2p4khz_spectrum_twosided.png"
    fig.savefig(path, dpi=DPI, facecolor="white")
    plt.close(fig)
    return path
...
def save_output_time(time_s: np.ndarray, output: np.ndarray) -> Path:
    return save_time_signal(
        ...
        color=SPECTRUM_BLUE,
        filename="07_audible_ring_modulated_output_time_20s.png",
    )
...
def save_output_spectrum(output: np.ndarray, sample_rate_hz: float) -> Path:
    ...
    style_spectrum_axis(ax)
    fig.subplots_adjust(left=0.10, right=0.98, bottom=0.18, top=0.86)
    path = OUTPUT_DIR / "08_audible_ring_modulated_output_spectrum_twosided.png"
    fig.savefig(path, dpi=DPI, facecolor="white")
    plt.close(fig)
    return path
...
def main() -> None:
    ...
    image_paths = [
        ...
        save_time_signal(
            time_s,
            carrier,
            title="Low-passed carrier",
            color=SIGNAL_BLACK,
            filename="03_low_passed_carrier_time_20s.png",
        ),
    ...
    audio_paths = [
        write_wav_mono(AUDIO_OUTPUT_DIR / "01_original_carrier_20s_48k.wav", sample_rate_hz, raw_carrier),
        write_wav_mono(AUDIO_OUTPUT_DIR / "02_low_passed_carrier_800hz_20s_48k.wav", sample_rate_hz, carrier),
        write_wav_mono(AUDIO_OUTPUT_DIR / "03_modulation_signal_2p4khz_20s_48k.wav", sample_rate_hz, modulation),
        write_wav_mono(AUDIO_OUTPUT_DIR / "04_audible_ring_modulated_output_20s_48k.wav", sample_rate_hz, output),
    ]
